...
public class PriorityJobDetailsExecutor extends AbstractSessionFactoryAwareExecutor {

    private final JobDAO jobDAO;

    public PriorityJobDetailsExecutor(AnalyticsRequest analyticsRequest, SessionFactory sessionFactory) {
        this(analyticsRequest, new JobDAO(), sessionFactory);
    }

    public PriorityJobDetailsExecutor(AnalyticsRequest analyticsRequest, JobDAO jobDAO,
...
//        final int pipelineCounterEnd = Integer.parseInt(param(PARAM_PIPELINE_COUNTER_END));
        ...
        List<Job> jobs = doInTransaction(sqlSession -> {
...
//            return jobDAO.getAllJobsByStageName(sqlSession, stageName, pipelineCounterStart, pipelineCounterEnd);
            return jobDAO.jobSummaryDetails(sqlSession, jobName, result);
        });
