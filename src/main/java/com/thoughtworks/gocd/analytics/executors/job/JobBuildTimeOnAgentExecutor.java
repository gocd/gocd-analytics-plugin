...
public class JobBuildTimeOnAgentExecutor extends AbstractSessionFactoryAwareExecutor {
    private final AnalyticsRequest analyticsRequest;
    private final JobDAO jobDAO;

    public JobBuildTimeOnAgentExecutor(AnalyticsRequest analyticsRequest, SessionFactory sessionFactory) {
        this(analyticsRequest, new JobDAO(), sessionFactory);
    }

    JobBuildTimeOnAgentExecutor(AnalyticsRequest analyticsRequest, JobDAO jobDAO, SessionFactory sessionFactory) {
    ...
    protected GoPluginApiResponse doExecute() {
        ...
        final String jobName = param(PARAM_JOB_NAME);

        List<Job> jobs = doInTransaction(new Operation<List<Job>>() {
            @Override
            public List<Job> execute(SqlSession sqlSession) {
                return jobDAO.jobDurationForAgent(sqlSession, agentUUID, pipelineName, stageName, jobName);
            }
        });

        Map<String, Object> data = new HashMap<>();
        data.put("jobs", jobs);
        data.put("identifier", param(PARAM_PIPELINE_NAME) + "/" + param(PARAM_STAGE_NAME) + "/" + param(PARAM_JOB_NAME));
        data.put("agent_uuid", agentUUID);
        data.put("agent_host_name", agentHostname);

        AnalyticsResponseBody responseBody = new AnalyticsResponseBody(data, "job_build_time_on_an_agent.html");
