...
public class AgentsWithResultsExecutor extends AbstractSessionFactoryAwareExecutor {

    private final AgentUtilizationDAO agentUtilizationDAO;

    public AgentsWithResultsExecutor(AnalyticsRequest analyticsRequest, SessionFactory sessionFactory) {
        this(analyticsRequest, new AgentUtilizationDAO(), sessionFactory);
    }

    AgentsWithResultsExecutor(AnalyticsRequest analyticsRequest, AgentUtilizationDAO agentUtilizationDAO, SessionFactory sessionFactory) {
    ...
    protected GoPluginApiResponse doExecute() {
        ...
        List<AgentResult> agentResults = doInTransaction(new Operation<List<AgentResult>>() {
            final int limit = 10;
            ...
            @Override
            public List<AgentResult> execute(SqlSession sqlSession) {
                return agentUtilizationDAO.agentResults(sqlSession, start, end, limit);
            }
        });

        AnalyticsResponseBody responseBody = new AnalyticsResponseBody(agentResults, "agent-with-results-chart.html");
