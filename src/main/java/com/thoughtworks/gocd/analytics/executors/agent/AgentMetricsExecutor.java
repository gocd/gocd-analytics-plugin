...
public class AgentMetricsExecutor extends AbstractSessionFactoryAwareExecutor {

    private final AgentUtilizationDAO agentUtilizationDAO;

    public AgentMetricsExecutor(AnalyticsRequest analyticsRequest, SessionFactory sessionFactory) {
        this(analyticsRequest, new AgentUtilizationDAO(), sessionFactory);
    }

    AgentMetricsExecutor(AnalyticsRequest analyticsRequest, AgentUtilizationDAO agentUtilizationDAO, SessionFactory sessionFactory) {
    ...
    protected GoPluginApiResponse doExecute() {
        ...
        List<AgentMetric> agentResults = doInTransaction(new Operation<List<AgentMetric>>() {
            final int limit = 10;
            ...
            @Override
            public List<AgentMetric> execute(SqlSession sqlSession) {
                return agentUtilizationDAO.agentMetrics(sqlSession);
            }
        });

        AnalyticsResponseBody responseBody = new AnalyticsResponseBody(agentResults, "agent-metrics-chart.html");
