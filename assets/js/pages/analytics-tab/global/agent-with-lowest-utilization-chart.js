...
const c = new Console('agents-with-lowest-utilization-chart.js', 'dev');
...
async function init(data) {
    settings["AgentsLeastUtilized"] = await header.getLongestWaitingPipelinesHeader(graphOneHeaderChangeHandler);

    if (data.length === 0) {
        footer.showMessage("No data to display", "INFO", true);
    } else {
        graphManager.initSeries('AgentsLeastUtilized', data, settings['AgentsLeastUtilized']);
    }
}
