...
class GraphManager {
    ...
    setDataStack(data) {
        this.dataStack[this.child.getSeriesIndex()] = {name: this.name, data: data};
    }
    ...
    handleClick() {
        ...
        this.chart.on('click', (params) => {
            ...
            const requestParamPoint = this.child.get_requestParamsPoint(params.dataIndex);
            console.log('request param point is ', requestParamPoint);

            // const index = this.dataStack.length - 1;
            // console.log('index = ', index);

            const requestParams = JobChartFactories.get(this.child.getNextGraphName()).params(requestParamPoint);
