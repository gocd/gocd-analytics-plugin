...
class JobsHighestWaitTime {
    ...
    prepareData(data) {
        ...
        data.forEach(m => {
            actualCategories.push([m.stage_name, m.job_name]);
            categories.push(truncateString(m.stage_name, this.settings.truncateOrder) + ' >> ' + truncateString(m.job_name, this.settings.truncateOrder));
